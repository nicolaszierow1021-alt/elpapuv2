...
export default async function AdminDashboard({ searchParams }: { searchParams: { q?: string } }) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-200 font-sans selection:bg-accent/30">
      {/* Top Navigation Admin */}
      <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center justify-center gap-2 px-3 h-8 rounded-lg bg-surface border border-border text-gray-400 hover:text-white hover:border-[#00d0d0]/50 transition-all text-xs font-semibold">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-background border border-border rounded-xl p-5 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-24 h-24 bg-accent/5 rounded-full blur-2xl -mr-10 -mt-10 group-hover:bg-accent/10 transition-colors"></div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
          ...
          <div className="xl:col-span-3 order-2 xl:order-1">
            <div className="bg-background border border-border rounded-2xl overflow-hidden shadow-2xl">
              <div className="p-5 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0d0d12]">
                <div>
              ...
              <div className="p-5 bg-background">
                {error ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {movies.map((movie) => (
                      <div key={movie.id} className="group relative rounded-xl overflow-hidden bg-surface border border-border hover:border-[#00d0d0]/50 transition-all duration-300">
                        <div className="aspect-[2/3] relative">
                          ...
                          <div className="absolute top-2 left-2 flex flex-col gap-1.5">
                            {movie.category && (
                              <span className={`inline-flex px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-md backdrop-blur-md border shadow-lg ${
                                ...
                                'bg-accent/20 text-accent-hover border-accent/30'
                              }`}>
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-3 flex flex-col justify-end min-h-[40%]">
                            <h3 className="text-white font-bold text-sm leading-tight line-clamp-2 mb-1 group-hover:text-accent transition-colors">{movie.title}</h3>
                          ...
                          <div className="absolute top-2 right-2 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 z-10">
                            <Link href={`/admin/edit/${movie.id}`}>
                              <button className="bg-surface/80 backdrop-blur-md p-2 rounded-lg text-white hover:bg-accent hover:text-black border border-border transition-colors shadow-lg" title="Editar">
                  ...
                  <div className="text-center py-20 bg-[#0d0d12] rounded-xl border border-dashed border-border">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-accent-hover font-mono">Activo</span>
                  </div>
                  <div className="h-1.5 bg-border rounded-full overflow-hidden">
